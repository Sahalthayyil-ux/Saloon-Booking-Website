...
if(isset($_POST['btn_payment']))
{
	...
	?>
	<script>
    window.location="PackagePay.php?bid=<?php echo $booking?>&amount=<?php echo $amount; ?>";
    </script>
	<?php
}
?>
...
?>
  <div class="invoice-container">
    ...
    <form method="post">
      ...
      <input type="hidden" name="booking_id" id="booking_id" value="<?php echo $data['packagebooking_id']?>" />
...
function calculate(){
	...
	amount.forEach(function(amt){
		tamt = parseFloat(amt.textContent.replace(/[^0-9.]/g, ""));
		subtotal = subtotal + tamt;
	});
	var balance = subtotal - aamt;
	subtotalamount.innerHTML = "₹" + subtotal.toFixed(2);
